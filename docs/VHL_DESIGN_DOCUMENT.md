...
-   **SyncManager**: Implements the synchronization protocol using MinIO blobs and hash verification.
-   **MCP Servers**:
...
-   **AOSM (Agent Orchestration State Machine)**: The top-level controller. It transitions between `IDLE`, `BOOTSTRAP`, `TRIGGER_ANA`, and `WAIT_FOR_ANA`.
-   **Archy**: Uses vision models (SAM3 segmentation + LLM) to translate schematic images into a **Shared Circuit Understanding Document (SCUD)**.
...
sequenceDiagram
    ...
    AOSM->>Archy: Run Bootstrap (SAM3 + LLM)
    Archy-->>AOSM: SCUD (Markdown)
    AOSM->>Lib: Run Librarian
...
1.  **HASH_REQUEST**: Request the hash of the remote resource.
2.  **HASH_RESPONSE**: Compare local vs remote.
3.  **TRANSFER**:
