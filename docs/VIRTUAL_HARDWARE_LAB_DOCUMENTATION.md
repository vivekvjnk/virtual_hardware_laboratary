...
- `eis_data.txt`: Raw simulation data (e.g., EIS data) if generated by the control script.
- `nyquist_plot.png`: Visualizations of the simulation results (e.g., Nyquist plot for EIS).
...
The documentation itself could be exposed in various ways:
1.  **Dedicated Endpoint:** The MCP server could have a specific endpoint (e.g., `/docs` or `/about`) that returns this documentation as a string.
2.  **Initial Connection Message:** For simpler clients, a condensed version or a pointer to the full documentation could be included in the initial handshake or welcome message upon client connection.
3.  **"help" Command:** A client could issue a "help" command that triggers the server to send this documentation.

Given the existing structure of `mcp_server.py`, the most suitable approach for exposing this documentation would likely involve adding a new handler or extending an existing one to serve this content. The exact implementation details will depend on the existing MCP protocol definition.
