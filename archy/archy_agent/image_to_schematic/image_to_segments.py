...
def run_schematic_segmentation_pipeline(
    output_dir:  Path,
    image_path: Path,
    prompt_keywords: List[str] = ["circuit", "schematic"],
    ...
    for idx, det in enumerate(detections):
        ...
        crop_filename = f"subckt_{idx:03d}_s{score_int:02d}.png"
    ...
    for idx, det in enumerate(detections):
        ...
        label = f"{crop_filename}"
        # A simple text offset to avoid drawing exactly on the border
        text_pos = (x1 - 180, y1 - 20)  # adjust as needed
        draw.text(text_pos, label, fill="red",stroke_width=0.5,spacing=7)
...
if __name__ == "__main__":
    result = run_schematic_segmentation_pipeline(output_dir=Path("ana_workspace/fn1/schematic_subcircuits"), image_path=Path("ana_workspace/fn1/bq79616.png"), confidence_threshold=0.27)
