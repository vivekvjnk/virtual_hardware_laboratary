...
class ArchyEvaluator(AbstractEvaluator):
    ...
    def check_rules(self) -> Tuple[str, str]:
        ...
        try:
            ...
            return "FAILURE", "No semantic operations found in the database."
        ...
        if author != "ARCHY" or op_name != "SCUD_GENERATION":
            return "FAILURE", f"Last operation is not SCUD_GENERATION by ARCHY. Found op_name={op_name}, author={author}."

        if status != "SUCCESS":
            return "FAILURE", f"Last SCUD_GENERATION operation by ARCHY did not complete successfully. Status: {status}."

        return "SUCCESS", "Archy workflow completed successfully."