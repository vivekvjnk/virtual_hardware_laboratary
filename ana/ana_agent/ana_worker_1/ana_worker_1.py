...
def run_ana_w1_agent(workspace:str,scud_path: str, schematic_images_path: str = None,library_path: str = None,circuit_name: str = None,previous_iteration_dir: str = None, observations: List[str] = None, conversation: Optional[Conversation] = None):
    """
    ...
    Args:
        ...
        conversation: Existing conversation object for continuation (optional)
    """
    ...
    if conversation is None:
        ...
        # If conversation is provided, we just send the observations as a new message
        if observations:
            user_message = "\n\nFollowing new observations/suggestions have been proposed:\n"
            for i, obs in enumerate(observations):
                user_message += f"{i+1}. {obs}\n"
            conversation.send_message(user_message)
        else:
            logger.warning("[run_ana_w1_agent] No new observations provided for existing conversation.")
